...
- Name: dashboard.spec.ts >> TC-053 — Verify Overall System Health display
- Location: tests/dashboard.spec.ts:111:1
...
Call log:
  - waiting for locator('button').filter({ hasText: 'Test Coverage' }).first()
    - locator resolved to <button class="flex items-center gap-2 py-3 text-sm font-medium border-b-2 transition-colors border-transparent text-muted-foreground hover:text-foreground">…</button>
...
- generic [ref=e2]:
  ...
  - alert [ref=e3]:
    - generic [ref=e4]:
      - img [ref=e5]
      - heading "New version available" [level=2] [ref=e7]
      - paragraph [ref=e8]: The app has been updated. Please reload to get the latest version.
